...
function generatePlans() {
  ...
  renderOptions(plans, input);
  selectPlan(plans[0], input);
}
...
function scoreCoverage(data) {
  ...
  const areaDifference = Math.abs(data.actualArea - data.targetArea) / data.targetArea;
  ...
  const offsetDifference = Math.abs(data.offset - idealOffset) / data.treeInterval;
...
function renderOptions(plans, input) {
  ...
  plans.forEach((plan, index) => {
    ...
    card.innerHTML = `
      ...
      <p class="muted">
        Tap this option to update the schematic below.
      </p>
    `;
...
function describePattern(plan) {
  ...
  const offsetText =
    plan.offset > 0
      ? `, staggered by ${plan.offset} tree${plan.offset > 1 ? "s" : ""} on each placement row`
      : "";
