...
function generatePlans() {
  ...
  for (
    ...
    for (
      let treeInterval = 1;
      treeInterval <= Math.min(30, input.treesPerRow);
      treeInterval++
    ) {
      for (let offset = 0; offset < treeInterval; offset++) {
        ...
        candidates.push({
          ...
          rateDiff,
          percentDiff,
          spacingScore: scoreSpacing(rowInterval, treeInterval, offset, input),
  ...
  const plans = removeDuplicatePlans([
    ...
    {
      label: "Best Staggered Coverage",
      note: "Best for spreading coverage evenly across the orchard space.",
      ...bestSpacing
    },
    {
...
function scoreSpacing(rowInterval, treeInterval, offset, input) {
  const betweenRowsFeet = rowInterval * input.rowSpacing;
  const betweenTreesFeet = treeInterval * input.treeSpacing;

  let score = 100;

  const spacingBalance = Math.abs(betweenRowsFeet - betweenTreesFeet);
  score -= spacingBalance * 2;

  const idealOffset = treeInterval / 2;
  const offsetQuality = Math.abs(offset - idealOffset);
  score -= offsetQuality * 8;
...
function getCrewInstructions(input) {
  if (input.crewSize === 1) {
    return "One person mode: walk Row 1 from the starting end to the far end, then walk Row 2 from the far end back to the starting end. Continue alternating row direction through the block.";
  }
  ...
  return `Crew mode: split rows by person. ${assignments.join("; ")}. Each person can work their assigned rows in an up-and-back pattern.`;
}
